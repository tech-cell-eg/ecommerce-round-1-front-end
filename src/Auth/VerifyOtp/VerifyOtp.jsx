...
export default function VerifyOtp() {
  ...
  <section className='grid grid-cols-12 gap-2 h-screen '>
    ...
    <div className='col-span-12 md:col-span-5 py-6 md:bg-white   bg-gray-100 bg-opacity-80  max-[766px]:absolute w-full h-full'>
        ...
        <Link to={"/forgetpassword"} className='text-lg flex items-center w-fit ' ><IoIosArrowBack className='mr-2'/> Back</Link>
        <h2 className='text-2xl font-bold'>Enter OTP</h2>
        <p className='text-gray-600 md:text-gray-400  text-lg max-[260px]:text-sm'>we have share a code of your register email address <br/><span>ayafarh@gmail.com</span></p>
    </div>
            <div className='flex items-center justify-between  w-[75%] '>
            <input
      type="text"
      maxLength="1"
      className="focus:ring-0 w-14 h-14 max-[370px]:h-10 max-[370px]:w-10  bg-transparent max-[260px]:h-8 max-[260px]:w-8 mr-2 text-center text-xl font-semibold border-2 border-gray-600 md:border-gray-200 rounded-lg focus:outline-none focus:border-black"
      required
    />
      <input
      type="text"
    ...
    />
      ...
      type="text"
      maxLength="1"
      className="focus:ring-0 w-14 h-14 max-[370px]:h-10 max-[370px]:w-10 bg-transparent max-[260px]:h-8 max-[260px]:w-8  text-center text-xl font-semibold border-2 border-gray-600 md:border-gray-200 rounded-lg focus:outline-none focus:border-black"
      required
    />
            </div>

            <div className='w-full  '>
                <button type='submit' className='bg-black text-white py-2 rounded-lg w-full hover:bg-gray-900 transition-all duration-200 ' onClick={handleVerifysuccess}>verify</button>
